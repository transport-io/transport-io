...
- **Chrome and Firefox over WebTransport.** Safari establishes a session and then never
  sends; the fallback reaches it after 5 seconds.
- **One fallback, emits only.** A WebSocket carries the emit lane where the contract declares
  it; calls and streams need WebTransport.
