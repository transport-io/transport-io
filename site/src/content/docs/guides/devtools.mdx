...
<Shot
  light={pageLight}
  dark={pageDark}
  alt="The chat example with the devtools panel open along the bottom of the page. The panel's bar reads connected on webtransport with its counters, and below it a list of frames: datagrams, dropped datagrams in the accent colour, a chat message going out and coming back, and a stream's open, request and responses. A side column lists one open stream and the drops by event."
>
  The panel under the chat example, over real QUIC. Every row is a frame that crossed the
  wire, or a datagram this client dropped.
</Shot>
