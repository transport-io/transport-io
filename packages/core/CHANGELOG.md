...
- 209f921: `client.observe(observer, { preview })` reports what a session is doing, which no browser
...
- 6d2a7c2: `transport-io dev <entry>` no longer leaves the entry running when the command is stopped by
...
- f1e57bf: An error never prints the query of the URL it dialled. A failed handshake's message named the
...
- b3d15cb: A WebTransport session that connects and then sends nothing before the application
...
- 634fdbc: `transport-io/websocket-transport` exports `connectWebSocket` and its options and nothing
...
- 133596d: The WebSocket fallback has a keepalive and an idle deadline. Each side sends an empty
...
- fad3bcf: The reference binding moves to `@fails-components/webtransport` 1.6.8, which fixes the
...
- 30b8e37: A session disposed or failed before its handshake completed left its handshake-deadline timer
