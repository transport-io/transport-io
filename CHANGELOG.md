...
**Stage 0.** `0.1.0` is the first release, so there is nothing to migrate from. `0.0.1` was
published from the same tree to claim the name and is not a release.

Per-release notes are generated by changesets into
[`packages/core/CHANGELOG.md`](packages/core/CHANGELOG.md). What follows is a hand-written
account of the pre-release period, covering why things ended up the way they did.

## 0.2.0

**Upgrading breaks the wire.** Lane values are renamed and the handshake carries the lane as
a literal string, so a `0.1.0` peer and a `0.2.0` peer fail to negotiate with
`WT_PROTOCOL_ERROR`. Both sides upgrade together; there is no rolling path. In contracts,
`lane: 'stream'` becomes `lane: 'reliable'` and `lane: 'datagram'` becomes
`lane: 'unreliable'`. No error code changed, no frame type changed, and `call()` behaves
exactly as before.

The feature is `stream()`: an event declaring `yields` answers with a sequence. Per-release
detail is in [`packages/core/CHANGELOG.md`](packages/core/CHANGELOG.md); the reasoning is ADR
0012 and D93.
