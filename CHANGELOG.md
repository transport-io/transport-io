...
who do not exist. `0.1.0` is the first release, so there is nothing to migrate from. `0.0.1`
was published from the same tree to claim the name and is not a release.

Per-release notes are generated by changesets into
[`packages/core/CHANGELOG.md`](packages/core/CHANGELOG.md). What follows is the hand-written
account of the pre-release period, kept because it says why rather than what.
