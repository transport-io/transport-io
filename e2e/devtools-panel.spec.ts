...
test('the panel shows a call, an emit and a datagram as they cross the wire', async ({
  ...
  expect(lines[2]).toContain('overflowDropped')
  expect(lines[3]).toBe('time\tsession\tdir\tlane\tkind\tevent\tstream\tsize\tseq\tpreview')
  expect(lines.length).toBeGreaterThan(4)
  for (const line of lines.slice(4)) expect(line).toContain('\tcursor\t')
