...
> **Not built yet.** `call()` and `stream()` are specified in `PROTOCOL.md` and are not
> implemented. This document covers what runs: contracts, both lanes, rooms, and the
> client and server surfaces. Nothing below is aspirational.
